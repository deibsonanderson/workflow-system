...
function fncFormCadastro(element) {
	...
	// Bloco localiza todos os campos da classo mensagem Aleta e exibe o span
	// correpondente
	...
	// Caso todos os campos obrigatorios tenham sido preenchido a ação sera
	// execultada
	if (valido == true) {
...
function fcnModalDeleteId(element) {
	...
	$('#question-confirm').attr('processoFluxoId',
			$(element).attr('processoFluxoId'));
}
/**
 * Função responsavel por gerar uma tela de confirmação se sim execulta a ação
 * caso nao apenas fecha a div
 */
function fncDeleteId(element) {
	...
	$.ajax({
		...
		type : 'POST',
		data : 'retorno=' + retorno + '&controlador=' + controlador
				+ '&funcao=' + funcao + '&id=' + id + '&processoFluxoId='
				+ processoFluxoId + '&id_processo=' + id_processo,
		success : function(result) {
			$('#' + retorno).html(result);
		},
		beforeSend : function() {
...
function getId(element) {
	...
	$.ajax({
		...
		type : 'POST',
		data : 'retorno=' + retorno + '&controlador=' + controlador
				+ '&funcao=' + funcao + '&id=' + id + '&ordem=' + ordem,
		success : function(result) {
			$('#' + retorno).html(result);
		},
		beforeSend : function() {
...
function getIdSelec(element, paramIn) {
	...
	$.ajax({
		...
		type : 'POST',
		data : 'retorno=' + retorno + '&controlador=' + controlador
				+ '&funcao=' + funcao + '&id=' + id + '&param=' + paramIn,
		success : function(result) {
			$('#' + retorno).html(result);
		},
		beforeSend : function() {
...
function getIdProcesso(element) {
	...
	var vencimento_processo_fluxo = $(element)
			.attr('vencimento_processo_fluxo');
	...
	$.ajax({
		...
		type : 'POST',
		data : 'retorno=' + retorno + '&controlador=' + controlador
				+ '&funcao=' + funcao + '&id=' + id + '&id_processo='
				+ id_processo + '&id_processo_fluxo=' + id_processo_fluxo
				+ '&ativo=' + ativo + '&atuante=' + atuante
				+ '&titulo_processo_fluxo=' + titulo_processo_fluxo
				+ '&vencimento_processo_fluxo=' + vencimento_processo_fluxo
				+ '&descricao_processo_fluxo=' + descricao_processo_fluxo
				+ '&valor_processo_fluxo=' + valor_processo_fluxo,
		success : function(result) {
			$('#' + retorno).html(result);
		},
		beforeSend : function() {
...
/**
 * Função responsavel por gerar as mensagem de aleta atraves do Slide
 * 
 */
function msgSlide(msg) {
	if (msg != "") {
		...
		case "18":
			...
			break;
		case "19":
			msg1 = "É necessário ter selecionado pelo menos um arquivo!";
			tipo = "close";
			break;
		default:
			msg1 = msg;
	...
	if (tipo == "check") {
		...
		$
				.blockUI({
					...
					// block controlled via css above)
					centerY : true,
 ...
 */
/*
 * function enterPressed(evn) { if (window.event && window.event.keyCode == 13) {
 * $('.formCadastro').click(); } else if (evn && evn.keyCode == 13) {
 * $('.formCadastro').click(); } } document.onkeypress = enterPressed;
 */
			...
			.tinymce(
					{
						...
						// "insertlayer,moveforward,movebackward,absolute,|,styleprops,|,cite,abbr,acronym,del,ins,attribs,|,visualchars,nonbreaking,template,pagebreak,|,forecolor,backcolor,|,insertdate,inserttime,preview,|,ltr,rtl",
	...
	for (var i = 0; i < array.length; i++) {
		...
		case "hidden":
			if ($(obj).attr('upload') == "imagem") {
				...
				$('[name=' + aux[0] + 'Link]').attr('id',
						'./img/imagemPadrao.jpg');
				$('[name=' + aux[0] + 'Atual]').attr('src',
						'./img/imagemPadrao.jpg');
				$('[name=' + aux[0] + 'Icone]')
						.attr('src', "img/notes-add.gif");
			} else if ($(obj).attr('upload') == "arquivo") {
				$('[name=' + aux[0] + ']', '#formCadastro').val('');
				$('span[name=' + aux[0] + 'Atual]')
						.html(
								"<br />Adicione um arquivo clicando no <img src='img/notes-add.gif' border='0' /> ao lado.");

				$('[name=' + aux[0] + 'Icone]').attr('title',
						"Clique para adicionar");
				$('[name=' + aux[0] + 'Icone]')
						.attr('src', "img/notes-add.gif");

				$('span[name=' + aux[0] + 'Atual]').css('cursor', 'default');
				$('span[name=' + aux[0] + 'Atual]').css('text-decoration',
						'none');
			}
			break;
		default:
	...
	$('#valueChange')
			.html(
					'<div class="input-group mb-3">'
							+ '<input id="input-valor" type="text" value="'
							+ valor
							...
							+ '<a href="#" onClick="inputUpdate('
							+ fluxoProcessoId
							+ ',\'1\')" class="dropdown-item">Positivo</a>'
							+ '<a href="#" onClick="inputUpdate('
							+ fluxoProcessoId
							+ ',\'0\')" class="dropdown-item">Negativo</a>'
							+ '</div>' + '</div>' + '</div>');
...
function inputUpdate(fluxoProcessoId, propriedade) {
	...
	$('#valueChange').html(
			'<div onclick="inputShow(\'' + fluxoProcessoId + '\',\'' + valor
					+ '\',\'' + propriedade + '\')">R$ '
					+ (propriedade == '1' ? '' : '-') + valor + '</div>');
	...
	if (valor == '') {
		...
		$.ajax({
			url : 'controlador.php',
			type : 'POST',
			data : 'controlador=' + controlador + '&funcao=' + funcao
					+ '&valor=' + valor + '&id=' + fluxoProcessoId
...
function inputUpdateProcessoFluxo(fluxoProcessoId, tipo) {
	...
	if (tipo != 'v' && tipo != 'f' && valor == '') {
		...
		$.ajax({
			url : 'controlador.php',
			type : 'POST',
			data : 'controlador=ControladorProcesso&funcao=' + funcao
					+ '&valor=' + valor + '&id=' + fluxoProcessoId,
			success : function(result) {
				if (tipo == 'v') {
		...
		$('.cd-timeline__block')
				.each(
						function() {
							...
							if (titulo != undefined
									&& titulo.trim().toUpperCase().indexOf(
											texto.trim().toUpperCase()) !== -1) {

								if (isDiv === false) {
									var html = '<div class="list-filter col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12" style="background-color: #ffffff;min-height: 2.5rem;border: 1px solid #d2d2e4;padding-top: 8px;" onClick="filtrarTimeline(\''
											+ titulo.trim()
											+ '\', true)" >'
											+ titulo.trim() + '</div>';
									$('#filter').append(html);
...
function listUp(id) {
	...
	if (origem != null && origem != 0) {
		...
		$("#sortable li:eq(" + origem + ")").after(
				$("#sortable li:eq(" + destino + ")"));
	}
}
...
function telaVisualizarEventosAgenda(dateText) {
	$
			.ajax({
				url : 'controlador.php',
				type : 'POST',
				data : 'retorno=div_agenda_retorno&controlador=controladorAgenda&funcao=telaVisualizarEventosAgenda&data='
						+ dateText,
				success : function(result) {
...
function telaVisualizarComentariosAgenda(dateText) {
	$
			.ajax({
				url : 'controlador.php',
				type : 'POST',
				data : 'retorno=div_comentario_retorno&controlador=controladorAgenda&funcao=telaVisualizarComentariosAgenda&data='
						+ dateText,
				success : function(result) {
			...
			.sortable(
					{
						...
						cursor : 'move',
						update : function() {
							var txt_data_cad = document
									.getElementById("txt_data_cad").value;
							var order = $(this).sortable("serialize")
									+ '&action=updateRecordsListings&txt_data_cad='
									+ txt_data_cad
									+ '&retorno=div_agenda_retorno&controlador=controladorAgenda&funcao=ordernarAgenda';
							$.ajax({
								url : 'controlador.php',
								type : 'POST',
								data : order,
...
function desativarAgenda(id, ativo) {
	...
	var dados = 'id='
			+ id
			+ '&retorno=div_central&controlador=controladorAgenda&funcao=alterarAgenda&txt_data_cad='
			+ fncDesformataData(txt_data_cad) + '&ativo=' + ativo;
	$.ajax({
		url : 'controlador.php',
		type : 'POST',
		data : dados,
...
function removerAgenda(id, arquivo) {
	...
	var dados = 'id='
			+ id
			+ '&retorno=div_central&controlador=controladorAgenda&funcao=excluirAgenda&data='
			+ txt_data_cad;
	$.ajax({
		url : 'controlador.php',
		type : 'POST',
		...
		success : function(result) {
			$.ajax({
				...
				type : 'GET',
				data : 'opcao=2&pastaArquivo=./arquivos/agenda/&arquivo='
						+ arquivo
			});
			$('#div_central').html(result);
		},
...
function telaModalAgendaProcessoFluxo(info) {
	$
			.ajax({
				url : 'controlador.php',
				type : 'POST',
				data : 'retorno=div_modal_agenda_retorno&controlador=controladorAgenda&funcao=telaModalAgendaProcessoFluxo&id='
						+ info.id
						+ '&id_processo_fluxo='
						+ info.id_processo_fluxo
						+ '&ativo='
						+ info.ativo
						+ '&atuante='
						+ info.atuante
...
function fncTelaModalComentariosProcessoFluxo(element) {
	...
	$
			.ajax({
				url : 'controlador.php',
				type : 'POST',
				data : 'retorno=div_modal_timeline_retorno&controlador=controladorProcesso&funcao=telaModalComentariosProcessoFluxo&id='
						+ id_processo
						+ '&titulo_processo_fluxo='
						+ titulo_processo_fluxo
						+ '&id_processo_fluxo='
						+ id_processo_fluxo,
				success : function(result) {
...
function fncTelaModalCadastrarProcessoFluxo(element) {
	...
	$
			.ajax({
				url : 'controlador.php',
				type : 'POST',
				data : 'retorno=div_modal_timeline_retorno&controlador=controladorProcesso&funcao=telaModalCadastrarProcessoFluxo&id='
						+ id_processo,
				success : function(result) {
					$('#div_modal_timeline_retorno').html(result);
					$.blockUI({
...
function fixTableLayout(tableName) {
	...
	$('#' + tableName + '_filter').addClass(
			'form-group col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12 mb-2');
	$('#' + tableName + '_filter input').addClass('form-control');
	$('#' + tableName + '_length').addClass(
			'form-group col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12 mb-2');
	$('#' + tableName + '_length select').addClass('form-control');

	$('#' + tableName + '_info').addClass(
			'form-group col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12 mb-2');
	$('#' + tableName + '_paginate').addClass(
			'form-group col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12 mb-2');
...
function recalcular(processoFluxoId) {
						...
						if ($(this).attr('id') == 'valor_' + processoFluxoId) {
							...
							$
									.ajax({
										url : 'controlador.php',
										type : 'POST',
										data : 'controlador=ControladorProcesso&funcao=atualizarValorFluxoProcesso&valor='
												+ valorUpdate.replace("-", "")
												+ '&id='
												+ processoFluxoId
												+ '&propriedade=' + propriedade,
	...
	$('#totalFechado').html(
			'R$ ' + valorMonetario(totalFechado.toFixed(2), '3'));
	$('#totalPositivo').html(
			'R$ ' + valorMonetario(totalPositivo.toFixed(2), '3'));
	$('#totalNegativo').html(
			'R$ ' + valorMonetario(totalNegativo.toFixed(2), '3'));
	$('#totalGeral').html(
			'R$ '
					+ valorMonetario(
							(totalPositivo + totalNegativo).toFixed(2), '3'));
	$('#provisaoTotalGeral').html(
			'R$ '
					+ valorMonetario(
							(provisao + (totalPositivo + totalNegativo))
									.toFixed(2), '3'));
}
...
function fncIncludeText(element) {
	if ($(element).val() != undefined && $(element).val() != '') {
		...
		var periodo = fncRecuperarMes((parseInt(data.substr(3, 2))) - 1) + ' '
		...
		case '1':
			$('#descricao').html(
					'Boleto (' + titulo_atividade + ') - ' + periodo);
			$('#categoria_anexo').val('boleto');
			break;
		case '2':
			$('#descricao').html(
					'Comprovante de pagamento (' + titulo_atividade + ') - '
							+ periodo);
			$('#categoria_anexo').val('comprovante');
			break;
		case '3':
			$('#descricao').html(
					'Fatura (' + titulo_atividade + ') - ' + periodo);
...
function fcnSelectAllFiles(){
	checked = $('#all-checkbox-file').prop('checked');
	$('.checkbox-file').prop('checked',checked);
}

function fncBaixarTodosArquivos() {
	var files = '';
	...
	if(files != ''){ 
		window.location = "download-zip.php?d=diretorio"+files;
	}else{
		msgSlide('19');
	}
